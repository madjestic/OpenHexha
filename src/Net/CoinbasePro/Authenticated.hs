...
module Net.CoinbasePro.Authenticated
  ( accounts
  , account
  , accountHistory
  , accountHolds
...
import qualified Net.CoinbasePro.Authenticated.API              as API
import           Net.CoinbasePro.Authenticated.Accounts         (Account,
                                                                 AccountHistory,
                                                                 AccountId (..),
                                                                 Fees, Hold,
                                                                 TrailingVolume (..))
import           Net.CoinbasePro.Authenticated.CoinbaseAccounts (CoinbaseAccount)
import           Net.CoinbasePro.Authenticated.Deposit          (CoinbaseDepositRequest (..),
                                                                 CryptoDepositAddress,
                                                                 DepositRequest (..),
                                                                 DepositResponse)
import           Net.CoinbasePro.Authenticated.Fills            (Fill)
import           Net.CoinbasePro.Authenticated.Limits           (Limits)
import           Net.CoinbasePro.Authenticated.Orders           (Order,
                                                                 PlaceOrderBody (..),
                                                                 STP, Status (..),
                                                                 Statuses (..),
                                                                 TimeInForce,
                                                                 statuses)
import           Net.CoinbasePro.Authenticated.Payment          (PaymentMethod,
                                                                 PaymentMethodId (..))
import           Net.CoinbasePro.Authenticated.Request          (CBAuthT (..),
                                                                 authRequest)
import           Net.CoinbasePro.Authenticated.Transfer         (Transfer,
                                                                 TransferType (..))
import           Net.CoinbasePro.Authenticated.Withdrawal       (CoinbaseWithdrawalRequest (..),
                                                                 CryptoWithdrawalRequest (..),
                                                                 CryptoWithdrawalResponse,
                                                                 WithdrawalFeeEstimateResponse (..),
                                                                 WithdrawalRequest (..),
                                                                 WithdrawalResponse)
import           Net.CoinbasePro.Request                        (RequestPath,
                                                                 emptyBody)


import           Net.CoinbasePro.Types                          (ClientOrderId (..),
                                                                 CryptoAddress (..),
                                                                 CurrencyType (..),
                                                                 OrderId (..),
                                                                 OrderType, Price,
